...
namespace Service
{
    public class TimeSheetsService
    {
        ...
        public TimeSheetsService(IPersonRepository personRepository)
        {
            _personRepository = personRepository;

            FillPersonRepoWithInitData().Wait();
        }
        ...
        {
            return await _personRepository.GetPersonByNameAsync(name, personParameters, token);
        }

        public async Task<IEnumerable<Person>> GetPersonsAsync(PersonParameters personParameters, CancellationToken token)
        {
            return await _personRepository.GetPersonsAsync(personParameters, token);
        }


        public async Task<Person> AddPersonAsync(Person person, CancellationToken token)
        {
            Person newPerson = await _personRepository.AddPersonAsync(person, token);
            return newPerson;
        }
        ...
        private async Task FillPersonRepoWithInitData()
        {
            var data = new List<Person>() {
                ...
                new Person { Id = 51, FirstName = "Melinda", LastName = "Jordan", Email = "catty@nonummyFusce.ca", Company = "US Nibh Vulputate Company", Age = 23 }
            };
            CancellationTokenSource cts = new CancellationTokenSource();

            await _personRepository.AddPersonRangeAsync(data, cts.Token);
        }
    }
}
